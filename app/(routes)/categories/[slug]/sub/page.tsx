...
const Page = ({ params }: any) => {
    ...
    useEffect(() => {
        const fetchProductBySlug = async () => {
            ...
            try {
                // Fetch all types
                const typeResponse = await fetch(`https://admin.raelli.az/api/products/type/?page_size=20`);
                if (!typeResponse.ok) {
                ...
                const typeDetailsResponse = await fetch(`https://admin.raelli.az/api/products/type/${typeId}/`);
                const typeDetailsData = await typeDetailsResponse.json();
                setTypeDetails(typeDetailsData);

                // Fetch product subtypes
                const productResponse = await fetch(`https://admin.raelli.az/api/products/type/${typeId}`);
